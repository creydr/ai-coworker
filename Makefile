...
SYSTEMTEST_REGISTRY ?= localhost:5002
SYSTEMTEST_SANDBOX_IMAGE ?= $(SYSTEMTEST_REGISTRY)/ai-coworker-systemtest-sandbox:latest

systemtest-registry:
	@docker container inspect -f '{{.State.Running}}' systemtest-registry 2>/dev/null | grep -q true || docker run -d --name systemtest-registry -p 5002:5000 registry:2
